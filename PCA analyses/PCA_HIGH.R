...
library(coda) #niet gelukt
library(combinat)
library(conditionz)
library(crul) #niet gelukt
library(data.table)
library(expm)
library(fastmatch) #niet gelukt
library(foreach)  #niet gelukt
library(ggrepel)
library(gtools)
library(httpcode) #niet gelukt
library(igraph) #niet gelukt
library(iterators) #niet gelukt
library(maps) #niet gelukt
library(mnormt)
library(numDeriv) #niet gelukt
library(phangorn) #niet gelukt
library(phytools) #niet gelukt
library(plotrix) #niet gelukt
library(plyr)
library(readxl)
library(rotl)
#----------------------------------Clean memory---------------------------------
...
PopQs<-read_excel("PCA_HIGH.xlsx")
...
pcaData1=log(PopQs[,c(2:11)])  # identify columns that will be used in PCA (lambda, R0, GT, mu etc.)
...
name1=0.3885   # PC1 axis: fill this in for your pca output: summary(pca)! fraction variance explained by first axis; this output should be copied from summary(pca) 
name2=0.21429   # PC2 axis: fill this in for your pca output: summary(pca)! fraction variance explained by second axis;this output should be copied from summary(pca)
...
write.csv(cbind(x$scores,PopQs$Species),"PCAscoresHIGH.csv",row.names=F)
...
PCbiplot <- function(PC, x="PC1", y="PC2") {
  ...
  datapc <- data.frame(varnames=colnames(PopQs[2:11]), PC$loadings) #    WARNING: adapt number of columns do your dataset                    
  ...
  plot <- plot + geom_segment(data=datapc[4:4,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="chartreuse")
  plot <- plot + geom_segment(data=datapc[5:5,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="aquamarine")
  plot <- plot + geom_segment(data=datapc[6:6,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="blue")
  plot <- plot + geom_segment(data=datapc[7:7,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="darkgray")
  plot <- plot + geom_segment(data=datapc[8:8,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="darkgreen") 
  plot <- plot + geom_segment(data=datapc[9:9,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="cyan")
  plot <- plot + geom_segment(data=datapc[10:10,], aes(x=0, y=0, xend=v1, yend=v2), arrow=arrow(length=unit(0.2,"cm")), size=1.5, color="deeppink1") 
