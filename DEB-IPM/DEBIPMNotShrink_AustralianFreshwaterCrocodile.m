function [E_Y, SSD, PopQs] = DEBIPMNotShrink_AustralianFreshwaterCrocodile(E_Ymin,E_Ymax,step,MatrixSize);
...
Lm = 200; % maximum disc width in cm 
kappa = 0.97; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 0.78; % juvenile mortality rate (per year)
mu_ad = 0.04; % adult mortality rate (per year)
rb = 0.10; % von Bertalanffy growth rate (per year)
Rm = 13; % maximum number of offspring produced per year at L_m
