function [E_Y, SSD, PopQs] = DEBIPMNotShrink_BlueShark(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 36; % cm used to define limits of matrix approximation
Lb = 45; % disc width at birth in cm 
Lp = 201; % disc width at puberty in cm
Lm = 383; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 0.64; % juvenile mortality rate (per year)
mu_ad = 0.23; % adult mortality rate (per year)
rb = 0.10; % von Bertalanffy growth rate (per year)
Rm = 33.0; % maximum number of offspring produced per year at L_m
