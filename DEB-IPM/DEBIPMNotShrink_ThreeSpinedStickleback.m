function [E_Y, SSD, PopQs] = DEBIPMNotShrink_ThreeSpinedStickleback(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 1.2; % cm used to define limits of matrix approximation
Lb = 1.50; % disc width at birth in cm 
Lp = 3.50; % disc width at puberty in cm
Lm = 8; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 3.11; % juvenile mortality rate (per year)
mu_ad = 1.47; % adult mortality rate (per year)
rb = 1.16; % von Bertalanffy growth rate (per year)
Rm = 1500; % maximum number of offspring produced per year at L_m
