function [E_Y, SSD, PopQs] = DEBIPMNotShrink_EasternFenceLizard(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 20; % cm used to define limits of matrix approximation
Lb = 25; % disc width at birth in cm 
Lp = 58; % disc width at puberty in cm
Lm = 80; % maximum disc width in cm 
kappa = 0.61; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 2.81; % juvenile mortality rate (per year)
mu_ad = 1.18; % adult mortality rate (per year)
rb = 0.414; % von Bertalanffy growth rate (per year)
Rm = 23; % maximum number of offspring produced per year at L_m
