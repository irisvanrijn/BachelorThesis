function [E_Y, SSD, PopQs] = DEBIPMNotShrink_TheTreeDtella(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 19.2; % cm used to define limits of matrix approximation
Lb = 24; % disc width at birth in cm 
Lp = 48; % disc width at puberty in cm
Lm = 57.25; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 1.609; % juvenile mortality rate (per year)
mu_ad = 0.20; % adult mortality rate (per year)
rb = 0.850; % von Bertalanffy growth rate (per year)
Rm = 2; % maximum number of offspring produced per year at L_m
