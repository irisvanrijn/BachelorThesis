function [E_Y, SSD, PopQs] = DEBIPMNotShrink_LeatherbackTurtle(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 5.6; % cm used to define limits of matrix approximation
Lb = 7; % disc width at birth in cm 
Lp = 121; % disc width at puberty in cm
Lm = 175; % maximum disc width in cm 
kappa = 0.92; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 4.42; % juvenile mortality rate (per year)
mu_ad = 0.09; % adult mortality rate (per year)
rb = 0.12; % von Bertalanffy growth rate (per year)
Rm = 140.01; % maximum number of offspring produced per year at L_m
