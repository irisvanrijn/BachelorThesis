function [E_Y, SSD, PopQs] = DEBIPMNotShrink_WanderingGarterSnake(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 150.16; % cm used to define limits of matrix approximation
Lb = 187.70; % disc width at birth in cm 
Lp = 400; % disc width at puberty in cm
Lm = 700; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 1.11; % juvenile mortality rate (per year)
mu_ad = 0.28768; % adult mortality rate (per year)
rb = 0.3957; % von Bertalanffy growth rate (per year)
Rm = 14.35; % maximum number of offspring produced per year at L_m
