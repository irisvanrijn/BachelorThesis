function [E_Y, SSD, PopQs] = DEBIPMNotShrink_Porbeagle(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 46.4; % cm used to define limits of matrix approximation
Lb = 58; % disc width at birth in cm 
Lp = 200; % disc width at puberty in cm
Lm = 280; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 0.12; % juvenile mortality rate (per year)
mu_ad = 0.21; % adult mortality rate (per year)
rb = 0.11; % von Bertalanffy growth rate (per year)
Rm = 7.50; % maximum number of offspring produced per year at L_m
