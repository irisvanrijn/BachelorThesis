function [E_Y, SSD, PopQs] = DEBIPMNotShrink_LoggerheadSeaTurtle(E_Ymin,E_Ymax,step,MatrixSize)
...
Lp = 84.7; % disc width at puberty in cm
Lm = 130; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 0.29; % juvenile mortality rate (per year)
mu_ad = 0.35; % adult mortality rate (per year)
rb = 0.769; % von Bertalanffy growth rate (per year)
Rm = 76.5; % maximum number of offspring produced per year at L_m
