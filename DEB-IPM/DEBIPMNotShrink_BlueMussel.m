function [E_Y, SSD, PopQs] = DEBIPMNotShrink_BlueMussel(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 0.24; % cm used to define limits of matrix approximation
Lb = 0.30; % disc width at birth in cm 
Lp = 1.75; % disc width at puberty in cm
Lm = 15; % maximum disc width in cm 
kappa = 0.80; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 0.19; % juvenile mortality rate (per year)
mu_ad = 0.24; % adult mortality rate (per year)
rb = 0.85; % von Bertalanffy growth rate (per year)
Rm = 16000000.00; % maximum number of offspring produced per year at L_m
