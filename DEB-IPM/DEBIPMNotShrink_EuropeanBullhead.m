function [E_Y, SSD, PopQs] = DEBIPMNotShrink_EuropeanBullhead(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 2.4; % cm used to define limits of matrix approximation
Lb = 3.0; % disc width at birth in cm 
Lp = 50; % disc width at puberty in cm
Lm = 180; % maximum disc width in cm 
kappa = 0.91; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 2.12; % juvenile mortality rate (per year)
mu_ad = 1.18; % adult mortality rate (per year)
rb = 0.40; % von Bertalanffy growth rate (per year)
Rm = 424.0; % maximum number of offspring produced per year at L_m
