function [E_Y, SSD, PopQs] = DEBIPMNotShrink_SleepyLizard(E_Ymin,E_Ymax,step,MatrixSize)
...
Lmin = 7.2; % cm used to define limits of matrix approximation
Lb = 9; % disc width at birth in cm 
Lp = 23; % disc width at puberty in cm
Lm = 32.80; % maximum disc width in cm 
kappa = 0.91; % fraction energy allocation to respiration (as opposed to reproduction)
mu_juv = 1.06; % juvenile mortality rate (per year)
mu_ad = 0.11; % adult mortality rate (per year)
rb = 0.50; % von Bertalanffy growth rate (per year)
Rm = 0.88; % maximum number of offspring produced per year at L_m
